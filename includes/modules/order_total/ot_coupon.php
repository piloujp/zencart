...
class ot_coupon extends base
{
    ...
    protected $_check;
    /**
     * $calculate_tax determines how tax should be applied to coupon tax_incl, tax_excl
     * @var string
     */
    ...
    function process()
    {
        ...
        if ($od_amount['total'] > 0) {
            ...
            $order->info['total'] -= zen_config('DISPLAY_PRICE_WITH_TAX') === 'true' ? $od_amount['total'] : $od_amount['total'] + $tax;
            // When discount module is displayed after tax module, add tax to amount, even when displaying prices tax excluded.
            if (zen_config('DISPLAY_PRICE_WITH_TAX') !== 'true' && !empty($order->info['tax_sort_order']) && $this->sort_order > $order->info['tax_sort_order']) {
                //$od_amount['total'] += $tax;
            }
            $order->info['tax'] -= $tax;
            ...
            if ((string)$orderAmountToCompareAgainstCouponMinimum >= $coupon_details['coupon_minimum_order']) {
                ...
                switch ($coupon_details['coupon_type']) {
                    case 'S': // Free Shipping
                        ...
                        $od_amount['tax'] = ($this->calculate_tax === 'tax_incl') ? $orderTotalDetails['shippingTax'] : 0;
                    ...
                    case 'F': // Fixed amount Off
                        if ($this->calculate_tax === 'tax_excl') {
                            ...
                            if ($this->include_shipping === 'true') {
                                $ratio = zen_config('DISPLAY_PRICE_WITH_TAX') === 'true' ? $od_amount['total'] / ($orderAmountTotal - $orderTotalDetails['orderTax']) : $od_amount['total'] / $orderAmountTotal;
                            } else {
                                $ratio = zen_config('DISPLAY_PRICE_WITH_TAX') === 'true' ? $od_amount['total'] / ($orderAmountTotal - $orderTotalDetails['orderTax'] + $orderTotalDetails['shippingTax']) : $od_amount['total'] / $orderAmountTotal;
                            }
                            $od_amount['total'] = zen_config('DISPLAY_PRICE_WITH_TAX') === 'true' ?  $orderAmountTotal * $ratio : $od_amount['total'];
                        } else {
                            if (zen_config('DISPLAY_PRICE_WITH_TAX') === 'true') {
                                $od_amount['total'] = ($coupon_details['coupon_amount'] > $orderAmountTotal ? $orderAmountTotal : $coupon_details['coupon_amount']) * ($orderAmountTotal > 0) * $coupon_product_count;
                            } else {                                
                            ...
                            if ($this->include_shipping === 'true') {
                                $ratio =  zen_config('DISPLAY_PRICE_WITH_TAX') === 'true' ?  $od_amount['total'] / $orderAmountTotal : $od_amount['total'] / ($orderAmountTotal + $orderTotalDetails['orderTax']);
                            } else {
                                $ratio =  zen_config('DISPLAY_PRICE_WITH_TAX') === 'true' ?  $od_amount['total'] / $orderAmountTotal : $od_amount['total'] / ($orderAmountTotal + $orderTotalDetails['orderTax'] - $orderTotalDetails['shippingTax']);
                            }
                            $od_amount['total'] = zen_config('DISPLAY_PRICE_WITH_TAX') === 'true' ?  $od_amount['total'] : $orderAmountTotal * $ratio;
                        }
                        break;
                    case 'O': // Both Fixed amount off & Free Shipping
                        if ($this->calculate_tax === 'tax_excl') {
                            ...
                            if ($this->include_shipping === 'true') {
                                $ratio = zen_config('DISPLAY_PRICE_WITH_TAX') === 'true' ? $od_amount['total'] / ($orderAmountTotal - $orderTotalDetails['shipping'] - ($orderTotalDetails['orderTax'] - $orderTotalDetails['shippingTax'])) : $od_amount['total'] / ($orderAmountTotal - $orderTotalDetails['shipping']);
                                $od_amount['total'] = zen_config('DISPLAY_PRICE_WITH_TAX') === 'true' ?  ($orderAmountTotal - $orderTotalDetails['shipping']) * $ratio + $orderTotalDetails['shipping'] : $od_amount['total'] + $orderTotalDetails['shipping'];
                            } else {
                                $ratio = zen_config('DISPLAY_PRICE_WITH_TAX') === 'true' ? $od_amount['total'] / ($orderAmountTotal - $orderTotalDetails['orderTax'] + $orderTotalDetails['shippingTax']) : $od_amount['total'] / $orderAmountTotal;
                                $od_amount['total'] = zen_config('DISPLAY_PRICE_WITH_TAX') === 'true' ?  $orderAmountTotal * $ratio + $orderTotalDetails['shipping'] : $od_amount['total'] + $orderTotalDetails['shipping'];
                            }
                        } else {
                            if (zen_config('DISPLAY_PRICE_WITH_TAX') === 'true') {
                                $od_amount['total'] = ($coupon_details['coupon_amount'] > $orderAmountTotal ? $orderAmountTotal : $coupon_details['coupon_amount']) * ($orderAmountTotal > 0) * $coupon_product_count;
                            ...
                            if ($this->include_shipping === 'true') {
                                $ratio = zen_config('DISPLAY_PRICE_WITH_TAX') === 'true' ? $od_amount['total'] / ($orderAmountTotal - $orderTotalDetails['shipping']) : $od_amount['total'] / ($orderAmountTotal + $orderTotalDetails['orderTax'] - $orderTotalDetails['shipping'] - $orderTotalDetails['ShippingTax']);
                                $od_amount['total'] = zen_config('DISPLAY_PRICE_WITH_TAX') === 'true' ?  $od_amount['total'] + $orderTotalDetails['shipping'] : ($orderAmountTotal - $orderTotalDetails['shipping']) * $ratio + $orderTotalDetails['shipping'];
                            } else {
                                $ratio = zen_config('DISPLAY_PRICE_WITH_TAX') === 'true' ? $od_amount['total'] / $orderAmountTotal : $od_amount['total'] / ($orderAmountTotal + $orderTotalDetails['orderTax'] - $orderTotalDetails['shippingTax']);
                                $od_amount['total'] = zen_config('DISPLAY_PRICE_WITH_TAX') === 'true' ?  $od_amount['total'] + $orderTotalDetails['shipping'] : $orderAmountTotal * $ratio + $orderTotalDetails['shipping'];
                            }
                        }
                        if (isset($_SESSION['shipping_tax_description']) && $_SESSION['shipping_tax_description'][0] != '') {
                            foreach ($_SESSION['shipping_tax_description'] as $ind => $descr) {
...
//    print_r($od_amount);
    ...
    function get_order_total($coupon_id): array
    {
        ...
        if ($this->include_shipping !== 'true') {
            ...
            if (!empty($_SESSION['shipping_tax_amount'])) {
                $orderTotalFull -= zen_config('DISPLAY_PRICE_WITH_TAX') === 'true' ? $order->info['shipping_cost'] - $order->info['shipping_tax'] : $order->info['shipping_cost'];
            }
        }
    ...
    function install()
    {
        ...
        $db->Execute("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function ,date_added) VALUES ('Re-calculate Tax', 'MODULE_ORDER_TOTAL_COUPON_CALC_TAX', 'tax_incl', 'Coupon amount is applied on subtotal tax included -> tax_incl (default), if applied on subtotal without tax -> tax_excl', '6', '7','zen_cfg_select_option(array(\'tax_incl\', \'tax_excl\'), ', now())");
    }
